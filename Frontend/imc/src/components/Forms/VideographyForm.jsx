...
/**
 * VideographyForm – same layout as PhotographyForm:
 * ✅ Add / View tabs
 * ✅ Search + pagination
...
const initialForm = {
  ...
  package_price: "",  // optional if you want to add price later
  payment_method: "Cash",
  notes: "",
};
...
const VideographyForm = ({ onClose, viewOnly = false }) => {
    ...
    try {
      ...
      setError(
        err?.response?.status === 404
          ...
          : err?.response?.data?.detail || err?.message || "Failed to load videography bookings."
      );
    } finally {
      setLoading(false);
    }
  ...
  // Payment single-select via tags
  ...
  const onDelete = async (row) => {
    if (!confirm(`Delete videography booking for ${row.project || row.client_name || "this client"}?`)) return;
    try {
