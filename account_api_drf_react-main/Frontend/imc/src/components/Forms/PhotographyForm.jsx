...
/**
 * PhotographyForm – simplified:
 * ✅ Shows only “Payment Method” section
 * 🚫 Removes “Payment & Status” and “Files” sections
 */

const BASE_API = import.meta.env.VITE_BASE_API_URL || "http://127.0.0.1:8000";
const CANDIDATE_API_URLS = [
  `${BASE_API}/api/auth/photography/`,
  `${BASE_API}/api/photography/`,
  `${BASE_API}/api/photography-bookings/`,
];
const PAGE_SIZE = 10;
...
const eventTypes = ["Wedding", "Pre-wedding", "Birthday", "Corporate", "Concert", "Model Shoot", "Other"];
...
const PhotographyForm = ({ onClose, viewOnly = false }) => {
    ...
    try {
      ...
      setError(
        err?.response?.status === 404
          ...
          : (err?.response?.data?.detail || err?.message || "Failed to load photography bookings.")
      );
    } finally {
      setLoading(false);
    }
  ...
  const buildPayload = () => {
    const alias = { client_name: "client", mobile_no: "contact_number", shoot_date: "date" };
    const p = {};
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      resetForm();
      if (tab === "VIEW") fetchRows(); else setTab("VIEW");
    } catch (err) {
      setError(
        err?.response?.status === 404
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const hay = `${r.client || r.client_name} ${r.contact_number || r.mobile_no} ${r.email} ${r.event_type} ${r.location} ${r.package_type}`.toLowerCase();
    ...
    <div className="pf-wrap">
      <div className="pf-header">
        ...
        <div className="pf-tabs">
          <button className={tab === "ADD" ? "active" : ""} onClick={() => setTab("ADD")}>Add Booking</button>
          <button className={tab === "VIEW" ? "active" : ""} onClick={() => setTab("VIEW")}>View Bookings</button>
        </div>
      </div>
        ...
        <form className="pf-form" onSubmit={onSubmit}>
          <section className="pf-card">
            ...
            <div className="pf-grid">
              <label>
                Client Name*
                <input name="client_name" value={form.client_name} onChange={onChange} placeholder="Full name" required />
              </label>
              <label>
                Email
                <input type="email" name="email" value={form.email} onChange={onChange} placeholder="client@email.com" />
              </label>
              <label>
                Mobile
                <input name="mobile_no" value={form.mobile_no} onChange={onChange} placeholder="98765 43210" />
              </label>
              <label>
                Event Type
          ...
          <section className="pf-card">
            ...
            <div className="pf-grid">
              <label>
                Shoot Date*
                <input type="date" name="shoot_date" value={form.shoot_date} onChange={onChange} required />
              </label>
              <label>
                Start Time*
              ...
              <label>
                Duration (hours)
                <input type="number" min="1" name="duration_hours" value={form.duration_hours} onChange={onChange} />
              </label>
              <label>
                Location*
                <input name="location" value={form.location} onChange={onChange} placeholder="Venue / Address" required />
              </label>
              <label>
                Photographers
              ...
              <div className="pf-tags">
                {methodOptions.map((m) => (
                  <button
                    type="button"
                    key={m}
                    className={form.payment_methods.includes(m) ? "tag active" : "tag"}
          ...
          <div className="pf-actions">
            <button className="btn" disabled={saving}>
              {editingId ? (saving ? "Updating..." : "Update Booking") : (saving ? "Saving..." : "Create Booking")}
            </button>
      ...
      {tab === "VIEW" && (
        <div className="pf-table-card">
              ...
              <tbody>
                {pageRows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="c">
                      <button className="mini" onClick={() => onEdit(r)}>Edit</button>
                      <button className="mini danger" onClick={() => onDelete(r)}>Delete</button>
                    </td>
                  </tr>
                ))}
                {!pageRows.length && (
                  <tr><td colSpan="6" className="c muted">{loading ? "Loading..." : "No records found."}</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="pf-pager">
            <button className="mini" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Prev</button>
            <span>Page {page} / {totalPages}</span>
            <button className="mini" disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>Next</button>
          </div>
        </div>
      )}
    </div>
