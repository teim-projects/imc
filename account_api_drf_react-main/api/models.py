...
class Studio(models.Model):
    ...
    time_slot = models.TimeField(blank=True, null=True)  # "HH:MM"
    ...
    # Payment Options (CSV; API can expose as list)
...
class Equipment(models.Model):
    ...
    rate_per_day = models.DecimalField(max_digits=10, decimal_places=2, default=0, validators=[MinValueValidator(0)])
...
class EquipmentRental(models.Model):
    ...
    rate_per_day_snapshot = models.DecimalField(max_digits=10, decimal_places=2, default=0, validators=[MinValueValidator(0)])
    ...
    def clean(self):
        ...
        # Sum all active rentals for that equipment on that date (exclude cancelled/returned)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.equipment and (self.rate_per_day_snapshot is None or self.rate_per_day_snapshot == 0):
            self.rate_per_day_snapshot = self.equipment.rate_per_day or 0
        # compute total (1 day minimum if return_date missing)
        days = 1
        if self.return_date and self.return_date >= self.rental_date:
            days = (self.return_date - self.rental_date).days or 1
        self.total_price = (self.rate_per_day_snapshot or 0) * self.quantity * days
